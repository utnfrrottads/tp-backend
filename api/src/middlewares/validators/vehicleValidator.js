...
const validateDataTypes = [
    check('licensePlate', "The license plate format is invalid. It must be 'ABC123' or 'AB123CD'.")
        .custom(value => {
    ...
    check('year', `The value must be a positive integer between 1990 and ${new Date().getFullYear()}.`)
        .isInt({ min: 1990, max: new Date().getFullYear() }),
    check('currentNumberOfKilometers', "The value must be a positive integer or decimal number.")
        .isFloat({ gt: 0 }),
    (req, res, next) => {
        validateResult(req, res, next);
    }
